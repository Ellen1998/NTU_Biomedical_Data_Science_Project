...
def train(args):
    ...
    #scheduler  = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=optimizer, T_0=5, T_mult=int(args.epoch/5), eta_min=1e-4)
    ...
    for num_epoch in range(args.epoch):
        ...
        print(f'Train result : Epoch:{num_epoch} , loss:{loss_item:.4f}')
        sys.stdout.flush()
        ...
        if mse < mse_best:
            torch.save(reg_model, './best_model_adam.pth')
            mse_best = mse
